...
from flight_forecast.bigquery_schema import RUNS_TABLE
from flight_forecast.bigquery_storage import _collection_table_path, settings
...
def format_report(
    ...
    lines.extend(
        [
            "",
            "raw保存のrun_idを確認し、`python -m flight_forecast.data_collector --replay-run-id <run_id>`で再生してください。",
        ]
    )
    return "\n".join(lines)
