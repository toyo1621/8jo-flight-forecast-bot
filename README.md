...
羽田空港から八丈島空港へ向かうANA便について、過去の運航実績と気象情報、現在の気象予報を組み合わせ、運航確率の参考値を表示するプロジェクトです。

**公開サイト:** [八丈島運航統計予測](https://toyo1621.github.io/8jo-flight-forecast-bot/)
...
- ANA1891・ANA1893・ANA1895について、当日から10日後までの運航確率を表示
- 便カードに主予報(Open-Meteo)の運航確率と、GFS・ECMWF・JMAの参考運航確率を表示
- 運航確率に「◎」「〇」「△」「×」の記号を付与
...
- 気象庁(JMA)のGSM・MSM予報から独立した参考運航確率を計算
- 日本周辺の短期予報ではJMAを重視しつつ、GFS・ECMWFも比較材料として表示
- 主予測とJMA参考値の差が20ポイント以上なら「気象モデル差に注意」を表示
- 外部の台風影響目安（JMAモード）から日別の飛行機向け影響度を取得し、「台風接近リスク」を表示
- 運航確率60%未満の便をオレンジで強調（警告の有無とは独立）
...
flowchart LR
    O[ODPT API] --> C[運航実績収集]
    M[Open-Meteo 標準予報] --> P[運航確率エンジン]
    E[GFS・ECMWF アンサンブル] --> P
    J[JMA GSM・MSM] --> P
    T[台風影響目安 JMAモード] --> P
    C --> Q[(BigQuery)]
    Q --> P
    S[(SQLite / SQL snapshot)] -. ローカル補助 .-> P
    P --> B[静的サイト生成]
    B --> G[GitHub Pages]
```
...
[Open-Meteo](https://open-meteo.com/en/docs)から、八丈島空港周辺（緯度33.115、経度139.782）と羽田空港周辺（緯度35.5494、経度139.7798）の次のデータを取得します。
...
JMA予報はGFS・ECMWFの62通りのアンサンブルには加えず、第三の独立した参考モデルとして使用します。日本周辺の短期予報ではJMAの影響を重く見る価値があるため、短期はJMA参考値と主予報との差を特に確認します。先の日程ではGFS・ECMWFも絡みやすくなるため、すべてのモデルを比較材料として残しています。

### 台風影響度

[八丈島・東京方面 台風影響目安](https://8jo-typhoon-ensemble.toyo1621.workers.dev/?source=jma)の公開JSON APIをJMAモードで取得し、日別の`targets.flight.riskLevel`を台風接近リスクに使用します。APIの`low`は補正なし、`medium`はリスク小、`high`はリスク中、`severe`はリスク大として扱います。取得できない場合は新しい台風補正を適用せず、直近3時間以内のキャッシュがあれば前回の影響度を使用します。
...
画面の主予報にはOpen-Meteo標準予報を使用します。GFS・ECMWF・JMAの値は、便カードと詳細画面で比較する参考運航確率です。
...
さらに視程不良、降水、低層雲、強風・突風、台風接近リスクの条件で確率を補正します。低層雲量80%超では0.9、95%以上では0.75を掛けます。航空会社都合や機材繰りなど気象以外の要因を考慮し、表示上限は97%です。

視程5 km未満では0.6、3 km未満では0.45を掛けます。降水量2 mm/h以上では0.85、8 mm/h以上では0.7を掛けます。突風と平均風速の補正は同時には掛けず、最大瞬間風速15 m/s以上を優先し、20 m/s以上では0.55、それ以外で平均風速10 m/s以上なら強風補正を掛けます。

台風接近リスクは外部サイトの日別の飛行機向け影響度から判定します。`low`は補正なし、`medium`はリスク小として0.9、`high`はリスク中として0.8、`severe`はリスク大として0.7を、主予報、GFS、ECMWF、JMAの表示確率に掛けます。同じ日の全便へ同じ影響度を適用します。
...
Open-MeteoのGFS・ECMWFアンサンブル予報を使い、複数の気象シナリオごとに運航確率を再計算します。その中央80%に含まれる確率の幅が狭いほど、天候信頼度を高く表示します。
...
便カードと詳細画面には、GFS・ECMWFそれぞれ31メンバーから算出した参考運航確率の中央値と、JMA予報での参考運航確率を表示します。JMA参考値はこのA〜E判定には含めません。短期はJMAを重視しつつ、GFS・ECMWFとの違いも見られるようにし、主予測との差が20ポイント以上の場合のみ、リスク欄へ「気象モデル差に注意」を表示します。

## 画面表示のルール

- 便カードの確率が60%未満の場合のみ、枠と確率をオレンジで表示します。
- 便カードの主予報確率には`(Open-Meteo主予報)`を添えます。
- 便カードにはGFS・ECMWF・JMAの参考運航確率も表示し、各モデル名の横にSVG旗アイコンを表示します。
...
本プロジェクトは、気象現象そのものを独自に予報することを目的としていません。第三者が提供する気象予報データを入力とし、過去の運航実績との類似性から運航確率の参考値を統計的に表示する試験的な取り組みです。
...
CSVの`運航`と旧表記の`通常`は`運航`、`条件付→運航`・`条件付き運航`・`条件付き→就航`は`運航(条件付)`としてDBへ保存します。通常運航か条件付き運航か判別できない場合も、飛んだ事実だけを示す`運航`を使用します。`欠航(強風)`のような括弧内の理由は`status_reason`へ分離します。Open-Meteo Archiveで取得できない過去視程は`backfill_bigquery_visibility.py`でHistorical Forecastから補完します。
...
```bash
python data_quality.py --backend sqlite --format markdown --output data_quality_report.md --fail-on none
```
...
- GitHub ActionsはCI、Pages公開、日次収集の3系統で実行します。各workflowにはtimeoutを設定し、実行が長時間ぶら下がらないようにしています。
...
- 依存関係はDependabotで週次確認し、Python依存とGitHub Actionsをグループ更新します。
