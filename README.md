...
CSVの`運航`と旧表記の`通常`は`運航`、`条件付→運航`は`条件付き運航`としてDBへ保存します。通常運航か条件付き運航か判別できない場合も、飛んだ事実だけを示す`運航`を使用します。`欠航(強風)`のような括弧内の理由は`status_reason`へ分離します。Open-Meteo Archiveで取得できない過去視程は`backfill_bigquery_visibility.py`でHistorical Forecastから補完します。
