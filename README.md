...
2. **気象データ**  
   [Open-Meteo API](https://open-meteo.com/) を使用し、八丈島空港（緯度: 33.115, 経度: 139.782）の指定日時の風向・風速・突風・低層雲量を取得します（APIキー不要）。
3. **データベース**  
...
2. **就航予測統計エンジンの作成**:
   - 蓄積された気象条件（特に「風速」と「風向」の掛け合わせ）から、就航確率を算出する統計スクリプトの実装。
3. **可視化ダッシュボードの構築**:
