...
- 便カードに主予報(Open-Meteo)とGFS・ECMWF・JMAによる統計参考値を表示
- 運航統計参考値に「◎」「〇」「△」「×」の記号を付与
...
- 気象庁(JMA)のGSM・MSM予報から独立した統計参考値を計算
...
flowchart LR
    O[ODPT API] --> C[運航実績収集]
    M[Open-Meteo 標準予報] --> P[運航統計参考値エンジン]
...
画面の主予報にはOpen-Meteo標準予報を使用します。GFS・ECMWF・JMAの値は、便カードと詳細画面で比較する統計参考値です。コード上の`probability`は既存互換のフィールド名であり、統計的に校正された確率を意味しません。
...
便カードと詳細画面には、GFS・ECMWFそれぞれ31メンバーから算出した統計参考値の中央値と、JMA予報による統計参考値を表示します。JMA参考値はこのA〜E判定には含めません。短期はJMAを重視しつつ、GFS・ECMWFとの違いも見られるようにし、主予測との差が20ポイント以上の場合のみ、リスク欄へ「気象モデル差に注意」を表示します。

## 画面表示のルール

- 便カードの確率が60%未満の場合のみ、枠と確率をオレンジで表示します。
- 便カードの主予報確率には`(Open-Meteo主予報)`を添えます。
- 便カードにはGFS・ECMWF・JMAの統計参考値も表示し、各モデル名の横にSVG旗アイコンを表示します。
