...
羽田発・八丈島行きのANA1891、ANA1893、ANA1895について、同じ便の過去実績と公開気象予報から、過去実績×リスクによる運航参考スコアを表示する静的サイトです。表示する%は将来の運航確率を保証するものではありません。
...
- **主予報**: Open-Meteoの`jma_seamless`。JMA GSM・MSMモデルを画面の大きな統計参考値に使用します。
- **補完予報**: JMAモデルが提供しない最大瞬間風速と視程だけをOpen-Meteo標準予報で補完します。JMA由来の値を上書きしてはいけません。
- **GFS**: 最大31メンバー。中央値を比較用の統計参考値として表示します。
- **ECMWF**: 最大31メンバー。中央値を比較用の統計参考値として表示します。
- **天候信頼度**: GFS 31 + ECMWF 31の最大62通りで計算します。
...
7. 視程3 km以上5 km未満は`× 0.8`、1 km以上3 km未満は`× 0.7`、1 km未満は`× 0.5`です。
...
11. 台風影響度の倍率はJMA主予報、GFS、ECMWFの各参考値へ適用します。
12. 表示範囲は0〜97%です。
...
- 画面上で「過去実績×リスクによる参考スコアで、将来の運航確率ではない」と明示します。
- 60%未満の便だけをオレンジにします。警告の有無でカード色を変えません。
- 主表示には`(JMA主予報 / 過去実績×リスクによる参考スコア)`を添えます。
- 比較欄にはGFS・ECMWF・JMAを併記し、すべてに「統計参考値」というラベルを使います。
- JMA比較欄は主予報の値とリスク要約を再利用し、別のJMA予報取得や再計算を行いません。
...
- 気象データの欠測と0を区別してください。
- 最大瞬間風速・視程の補完やアンサンブルの一部障害と、JMA主予報全体の障害を区別してください。
