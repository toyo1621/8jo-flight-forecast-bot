...
4. `運航`、`遅延`、`条件付き→就航`は1.0、それ以外は0.0です。旧データの`通常`も後方互換で1.0として扱います。
5. 視程5 km未満は`× 0.6`です。
6. 低層雲量90%超は`× 0.9`です。
...
- DB保存時は`運航`と旧表記の`通常`を`運航`、`条件付→運航`を`条件付き運航`へ正規化します。条件付きか判別できない運航は`運航`のままにします。表示時のみ`条件付き運航`を`条件付き→就航`へ変換します。
