...
def test_build_daily_forecasts_evaluates_each_ensemble_member_once():
    ...
    with (
        patch("flight_forecast.web_app.predict_flight_probability", side_effect=predictor),
        patch("flight_forecast.web_app.find_similar_flights", return_value=[]),
    ):
        build_daily_forecasts(
            SAMPLE_WEATHER,
...
def test_main_forecast_uses_jma_and_supplements_unavailable_fields():
    ...
    with patch(
        "flight_forecast.web_app.requests.get",
...
def test_main_forecast_keeps_jma_when_supplement_is_unavailable():
    ...
    with patch(
        "flight_forecast.web_app._fetch_deterministic_forecast",
...
def test_forecast_cache_write_failure_does_not_discard_live_forecast():
    with (
        patch("flight_forecast.web_app.fetch_forecast", return_value=SAMPLE_WEATHER),
        patch("flight_forecast.web_app.fetch_ensemble_forecast", return_value={}),
        patch("flight_forecast.web_app.fetch_typhoon_impacts", return_value={}),
        patch("flight_forecast.web_app.save_forecast_bundle", side_effect=OSError("read-only cache")),
    ):
...
def test_typhoon_impacts_use_jma_flight_risk_levels():
    ...
    with patch("flight_forecast.web_app.requests.get", return_value=response) as get:
...
def test_typhoon_impacts_do_not_fall_back_to_summary_risk():
    ...
    with patch("flight_forecast.web_app.requests.get", return_value=response):
        impacts = fetch_typhoon_impacts()
...
def test_typhoon_numeric_adjustment_can_be_disabled_for_small_samples():
    ...
    with patch("flight_forecast.web_app.TYPHOON_NUMERIC_ADJUSTMENT_ENABLED", False):
        adjusted = _with_typhoon_impact(result, typhoon_impact("severe"))
...
def test_today_flight_is_explicitly_marked_after_arrival_plus_30_minutes():
    ...
    with (
        patch("flight_forecast.web_app.predict_flight_probability", return_value={"probability": 88.0}),
        patch("flight_forecast.web_app.find_similar_flights", return_value=[]),
    ):
...
def test_load_forecast_bundle_uses_cached_main_forecast_on_api_error():
    ...
    with (
        ...
        patch("flight_forecast.web_app.load_cached_forecast_bundle", return_value=cached),
        patch("flight_forecast.web_app.save_forecast_bundle") as save,
    ):
...
def test_load_forecast_bundle_reports_missing_primary_supplement():
    ...
    with (
        patch("flight_forecast.web_app.fetch_forecast", return_value=weather),
        patch("flight_forecast.web_app.fetch_ensemble_forecast", return_value={}),
        patch("flight_forecast.web_app.fetch_typhoon_impacts", return_value={"2026-06-20": "low"}),
        patch("flight_forecast.web_app.load_cached_forecast_bundle", return_value=None),
        patch("flight_forecast.web_app.save_forecast_bundle", return_value={}),
    ):
        bundle = load_forecast_bundle()
...
def test_load_forecast_bundle_reuses_cached_optional_sources():
    ...
    with (
        patch("flight_forecast.web_app.fetch_forecast", return_value=SAMPLE_WEATHER),
        patch("flight_forecast.web_app.fetch_ensemble_forecast", side_effect=ValueError("bad ensemble")),
        patch("flight_forecast.web_app.fetch_typhoon_impacts", side_effect=ValueError("bad typhoon")),
        patch("flight_forecast.web_app.load_cached_forecast_bundle", return_value=cached),
        patch("flight_forecast.web_app.save_forecast_bundle") as save,
    ):
        bundle = load_forecast_bundle()
...
def test_partial_typhoon_coverage_is_reported():
    ...
    with (
        patch("flight_forecast.web_app.fetch_forecast", return_value=weather),
        patch("flight_forecast.web_app.fetch_ensemble_forecast", return_value={}),
        patch("flight_forecast.web_app.fetch_typhoon_impacts", return_value={"2026-07-15": "low"}),
        patch("flight_forecast.web_app.load_cached_forecast_bundle", return_value=None),
        patch("flight_forecast.web_app.save_forecast_bundle", return_value={}),
    ):
        bundle = load_forecast_bundle()
...
def test_index_renders_forecast():
    ...
    with (
        ...
        patch("flight_forecast.web_app.predict_flight_probability", return_value=result),
        patch("flight_forecast.web_app.find_similar_flights", return_value=[]),
        patch("flight_forecast.web_app._flight_display_expired", return_value=False),
...
def test_index_uses_injected_jst_clock_for_today_selection():
    ...
    with (
        patch("flight_forecast.web_app.load_forecast_bundle", return_value=bundle),
        patch("flight_forecast.web_app.build_daily_forecasts", return_value=[day]) as build_days,
        patch("flight_forecast.web_app.render_template", return_value="ok") as render,
    ):
