...
def test_verify_public_artifact_requires_the_expected_artifact_id():
    ...
    with patch("flight_forecast.publish_prediction_snapshots.urlopen", return_value=response):
...
def test_verify_public_artifact_rejects_stale_html():
    ...
    with (
        patch("flight_forecast.publish_prediction_snapshots.urlopen", return_value=response),
...
def test_verify_public_artifact_retries_until_pages_propagates():
    ...
    with (
        patch("flight_forecast.publish_prediction_snapshots.urlopen", side_effect=[stale, current]),
