...
def test_first_flight_weather_uses_configured_eight_oclock_hour():
    with patch("flight_forecast.data_collector.requests.get", return_value=_weather_response()):
...
def test_odpt_request_failure_raises_without_secret_in_message():
    ...
    with (
        patch("flight_forecast.data_collector.requests.get", return_value=failed),
...
def test_odpt_transient_failure_is_retried_and_raw_payload_can_be_captured():
    ...
    with (
        patch("flight_forecast.data_collector.requests.get", side_effect=[first, second]) as get,
        patch("flight_forecast.data_collector.time.sleep") as sleep,
    ):
...
def test_odpt_request_uses_supported_filters_and_applies_target_date_locally():
    ...
    with patch("flight_forecast.data_collector.requests.get", return_value=response) as get:
...
def test_replay_collection_run_rebuilds_rows_from_raw_payloads():
    ...
    with (
        patch("flight_forecast.data_collector.load_raw_collection_payloads", return_value=raw_rows),
        patch("flight_forecast.data_collector.save_collected_data", return_value=3) as save,
...
def test_cleanup_only_does_not_call_external_apis(monkeypatch):
    ...
    with (
        patch(
            "flight_forecast.data_collector.cleanup_unresolved_status_rows",
            return_value={"audit_id": "audit-1", "matched_count": 2, "affected_count": 0},
        ) as cleanup,
        patch("flight_forecast.data_collector.get_flight_data_odpt") as fetch_flights,
        patch("flight_forecast.data_collector.get_weather_data") as fetch_weather,
    ):
        main()
...
def test_collection_failure_does_not_write_bigquery(monkeypatch):
    ...
    with (
        patch(
            "flight_forecast.data_collector.get_flight_data_odpt",
            side_effect=CollectionError("ODPT APIからのデータ取得に失敗しました。"),
        ),
        patch("flight_forecast.data_collector.record_collection_run") as record_run,
...
def test_collection_date_argument_is_recorded_and_passed_to_odpt(monkeypatch):
    ...
    with (
        ...
        patch("flight_forecast.data_collector.get_weather_data", return_value=weather),
        patch("flight_forecast.data_collector.record_collection_run") as record_run,
        patch("flight_forecast.data_collector.save_collected_data", return_value=3),
    ):
        main()
