...
from flight_forecast.collection_outcomes import can_replace, classify
...
from flight_forecast.migrate_collection_outcomes import migration_sql
...
def test_weather_outage_keeps_final_flight_and_run_metadata(monkeypatch):
    ...
    with (patch('flight_forecast.data_collector.get_flight_data_odpt', return_value=rows),
          patch('flight_forecast.data_collector.get_weather_data', side_effect=CollectionError('weather missing')),
          patch('flight_forecast.data_collector.save_collected_data', return_value=1) as save,
          patch('flight_forecast.data_collector.record_collection_run') as run):
        main()
...
def test_no_final_flights_never_writes(monkeypatch):
    monkeypatch.setattr('sys.argv', ['data_collector.py', '--date', '2026-09-08'])
    monkeypatch.setenv('ODPT_API_KEY', 'fake')
    with (patch('flight_forecast.data_collector.get_flight_data_odpt', return_value=[]),
          patch('flight_forecast.data_collector.save_collected_data') as save,
          patch('flight_forecast.data_collector.record_collection_run') as run):
        main()
...
def test_replay_without_original_fetch_time_is_refused():
    with patch('flight_forecast.data_collector.load_raw_collection_payloads', return_value=[
...
def test_migration_preview_does_not_connect_to_bigquery(monkeypatch):
    from flight_forecast.migrate_collection_outcomes import main as migrate
    monkeypatch.setattr('sys.argv', ['migrate_collection_outcomes.py', '--protect-owner-report'])
    with patch('flight_forecast.migrate_collection_outcomes.bigquery.Client') as client:
        migrate()
    client.assert_not_called()


def test_storage_rejects_pending_even_if_status_says_operated():
    from flight_forecast.bigquery_storage import _normalize_item
...
def test_duplicate_conflict_does_not_discard_other_flight():
    from flight_forecast.data_collector import merge_with_daily_schedule
...
def test_fixture_fetch_to_archive_html_keeps_unknown_separate(monkeypatch):
    ...
    with (patch('flight_forecast.data_collector.get_flight_data_odpt', return_value=rows),
          patch('flight_forecast.data_collector.get_weather_data', side_effect=CollectionError('missing')),
          patch('flight_forecast.data_collector.save_collected_data', side_effect=save),
          patch('flight_forecast.data_collector.record_collection_run')):
        main()
